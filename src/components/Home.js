import React, { useEffect, useState } from "react";
import axios from "axios";
import { Link } from "react-router-dom";
export default function Home() {
  const [dataPipe, setDataPipe] = useState([]);

  useEffect(() => {
    axios
      .get("https://json-server-pipe.vercel.app/pipes")
      //renovoi d'une reponse si la requette est reussie, la reponse contient data
      .then((res) => setDataPipe(res.data))
      //erreur si non
      .catch((err) => console.log(err));
  }, []);

  const handleDelete = (id) => {
    const confirm = window.confirm("Would you like to delete");
    if (confirm) {
      axios
        .delete("https://json-server-pipe.vercel.app/pipes/" + id)
        .then((res) => {
          //au lieu d'utiliser navigate('/')
          window.location.reload();
        })
        .catch((err) => console.log(err));
    }
  };
  return (
    <div className="d-flex flex-column justify-content-center align-items-center bg-light vh-100">
      <h1>List of Pipes</h1>
      <div className="w-75 rounded bg-white border shadow p-4">
        <div className="d-flex justify-content-end">
          <Link to="/create" className="btn btn-success">
            Add+
          </Link>
        </div>
        <table className="table table-striped">
          <thead>
            <tr>
              {/* <th>Id</th> */}
              <th>Pre-Order Number</th>
              <th>Line Item</th>
              <th>Heat</th>
              <th>Pipe Number</th>
              <th>Pipe Length</th>
            </tr>
          </thead>
          <tbody>
            {/* Afficher les données ici */}
            {dataPipe.map((pipe) => (
              <tr key={pipe.id}>
                {/* <td>{pipe.id}</td> */}
                <td>{pipe.pre_order}</td>
                <td>{pipe.line_item}</td>
                <td>{pipe.heat}</td>
                <td>{pipe.pipe_number}</td>
                <td>{pipe.pipe_length}</td>

                <td>
                  <Link
                    to={`/read/${pipe.id}`}
                    className="btn btn-sm btn-info me-2"
                  >
                    Read
                  </Link>
                  <Link
                    to={`/update/${pipe.id}`}
                    className="btn btn-sm btn-primary me-2"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={(e) => handleDelete(pipe.id)}
                    className="btn btn-sm btn-danger "
                  >
                    Delete
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
